...
int main()
{
    ...
    timer.set_dim(m, n, k);
    timer.start();
    tsmm_reorder_ikj<double, m, n, k>(lhs.data_ptr(), rhs.data_ptr(), output.data_ptr());
    timer.stop();
    timer.print_result();
    ...
    timer.set_dim(m, n, k);
    timer.start();
    tsmm_simd_optimized<double, m, n, k, 64, 128, 16>(lhs.data_ptr(), rhs.data_ptr(), output.data_ptr());
    timer.stop();
    timer.print_result();
    ...
    timer.set_dim(m, n, k);
    timer.start();
    tsmm_unroll<double, m, n, k, 64, 128, 16>(lhs.data_ptr(), rhs.data_ptr(), output.data_ptr());
    timer.stop();
    timer.print_result();
