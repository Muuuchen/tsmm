...
void packing_a_k10(double *src, double *dst, int leading_dim, int dim_first,
                   int dim_second) {
...
void packing_b_k10(double *src, double *dst, int leading_dim, int dim_first,
                   int dim_second) {
...
void kernel_n_8_k10(double *a_buffer, double *b_buffer, double *c_ptr, int m,
...
void kernel_n_4_k10(double *a_buffer, double *b_buffer, double *c_ptr, int m,
                    int K, int LDC, double alpha) {
  int m_count, m_count_sub;
  int i, j, k;
  double *C = c_ptr;
...
void macro_kernel_k10(double *a_buffer, double *b_buffer, int m, int n, int k,
  ...
  for (n_count_sub = n, n_count = 0; n_count_sub > 7;
    ...
    kernel_n_8_k10(a_buffer, b_buffer + n_count * k, C + n_count * LDC, m, k,
                   LDC, alpha);
  }
  for (; n_count_sub > 3; n_count_sub -= 4, n_count += 4) {
    // call the m layer with n=4
    kernel_n_4_k10(a_buffer, b_buffer + n_count * k, C + n_count * LDC, m, k,
                   LDC, alpha);
  }
...
void mydgemm_cpu_v10(int M, int N, int K, double alpha, double *A, int LDA,
                     double *B, int LDB, double beta, double *C, int LDC) {

  if (beta != 1.0)
    scale_c_k10(C, M, N, LDC, beta);
  double *b_buffer =
  ...
  for (n_count = 0; n_count < N; n_count += n_inc) {
    ...
    for (k_count = 0; k_count < K; k_count += k_inc) {
      ...
      for (m_count = 0; m_count < M; m_count += m_inc) {
        ...
        macro_kernel_k10(a_buffer, b_buffer, m_inc, n_inc, k_inc,
