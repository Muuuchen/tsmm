...
void packing_a_k11(double *src, double *dst, int leading_dim, int dim_first,
                   int dim_second) {
...
void packing_b_k11(double *src, double *dst, int leading_dim, int dim_first,
                   int dim_second) {
...
#define macro_packing_kernel_1xkx8_v2                                          \
  ...
  for (k = k_start; k < k_end; k++) {                                          \
    ...
    sb0 = *(ptr_packing_b0);                                                   \
    sb1 = *(ptr_packing_b0 + 1);                                               \
    sb2 = *(ptr_packing_b1);                                                   \
    sb3 = *(ptr_packing_b1 + 1);                                               \
    sb4 = *(ptr_packing_b2);                                                   \
    sb5 = *(ptr_packing_b2 + 1);                                               \
    sb6 = *(ptr_packing_b3);                                                   \
    sb7 = *(ptr_packing_b3 + 1);                                               \
    sc0 += sa * sb0;                                                           \
    sc1 += sa * sb1;                                                           \
    sc2 += sa * sb2;                                                           \
    sc3 += sa * sb3;                                                           \
...
void kernel_n_8_v2_k11(double *a_buffer, double *b_buffer, double *c_ptr, int m,
                       int K, int LDC, double alpha) {
  ...
  // print_matrix(C,m,8);
  // printf("*****\n");
  for (m_count_sub = m, m_count = 0; m_count_sub > 23;
       m_count_sub -= 24, m_count += 24) {
...
void kernel_n_4_v2_k11(double *a_buffer, double *b_buffer, double *c_ptr, int m,
                       int K, int LDC, double alpha) {
  int m_count, m_count_sub;
  int i, j, k;
  double *C = c_ptr;
...
void macro_kernel_k11(double *a_buffer, double *b_buffer, int m, int n, int k,
  ...
  for (n_count_sub = n, n_count = 0; n_count_sub > 7;
    ...
    kernel_n_8_v2_k11(a_buffer, b_buffer + n_count * k, C + n_count * LDC, m, k,
                      LDC, alpha);
  }
  for (; n_count_sub > 3; n_count_sub -= 4, n_count += 4) {
    // call the m layer with n=4
    kernel_n_4_v2_k11(a_buffer, b_buffer + n_count * k, C + n_count * LDC, m, k,
                      LDC, alpha);
  }
...
void mydgemm_cpu_v11(int M, int N, int K, double alpha, double *A, int LDA,
                     double *B, int LDB, double beta, double *C, int LDC) {
  if (beta != 1.0)
    scale_c_k11(C, M, N, LDC, beta);
  double *b_buffer =
  ...
  for (n_count = 0; n_count < N; n_count += n_inc) {
    ...
    for (k_count = 0; k_count < K; k_count += k_inc) {
      ...
      for (m_count = 0; m_count < M; m_count += m_inc) {
        ...
        macro_kernel_k11(a_buffer, b_buffer, m_inc, n_inc, k_inc,
