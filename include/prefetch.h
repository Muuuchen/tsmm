...
static void InnerKernel(int m, int n, int k, double *a, int lda, double *b,
                        int ldb, double *c, int ldc, int first_time);
static void PackMatrixA(int, double *, int, double *);
static void PackMatrixB(int, double *, int, double *);

void tsmm_prefetch(int m, int n, int k, double *a, int lda, double *b, int ldb,
                   double *c, int ldc) {
  int i, p, pb, ib;
  for (p = 0; p < k; p += kc) {
    pb = min_(k - p, kc);
...
void PackMatrixB(int k, double *b, int ldb, double *b_to) {
  double *bj0 = &B(0, 0), *bj1 = &B(0, 1), *bj2 = &B(0, 2), *bj3 = &B(0, 3),
         *bj4 = &B(0, 4), *bj5 = &B(0, 5), *bj6 = &B(0, 6), *bj7 = &B(0, 7);
  for (int i = 0; i < k; i++) {
    *b_to++ = *bj0++;
    *b_to++ = *bj1++;
    *b_to++ = *bj2++;
