...
// required : (m,n,k)=(4000,16000,128),(8,16,16000),
// (32,16000,16),(144,144,144） optional :
// (m,n,k)=(16,12344,16),(4,64,606841),(442,193,11),(40,1127228,40) row major
template <typename T>
void tsmm(T *lhs, T *rhs, T *output, int M, int N, int K) {
  for (int i = 0; i < M; i++) {
    for (int j = 0; j < N; j++) {
...
template <typename T>
void tsmm_reorder_ikj(T *lhs, T *rhs, T *output, int M, int N, int K) {
...
// 明显差于 ikj
template <typename T>
void tsmm_reorder_kji(T *lhs, T *rhs, T *output, int M, int N, int K) {
  // reorder use cache line
  // kji
  for (int k = 0; k < K; k++) {
    for (int j = 0; j < N; j++) {
...
template <typename T>
void tsmm_block(T *lhs, T *rhs, T *output, int M, int N, int K, int BM, int BN,
                int BK) {
  for (int i = 0; i < M; i += BM) {
    for (int k = 0; k < K; k += BK) {
      for (int j = 0; j < N; j += BN) {
        ...
        for (int ii = i; ii < i_end; ++ii) {
          for (int kk = k; kk < k_end; ++kk) {
            ...
            for (int jj = j; jj < j_end; ++jj) { // ✅ 使用 j_end
              output[ii * N + jj] += temp * rhs[kk * N + jj];
            }
          }
        }
...
void blas_tsmm(const char trans_a, // 'N' 或 'T'
               ...
               T *c,               // 输出矩阵C
               const int ldc) {    // C的主维度
  std::cerr << "\033[32m blas_tsmm_naive not use scale factor \033[0m"
            << std::endl;
}
// 循环重排