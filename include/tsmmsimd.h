...
// 可以考虑使用循环展开来减少循环次数，从而减少代码大小。例如，将循环展开为 4 次，每次处理 4 个元素，这样可以减少循环次数，从而减少代码大小。
template <typename T, int M, int N, int K, int BM, int BN, int BK>
void tsmm_unroll(T *lhs, T *rhs, T *output)
{
    static_assert(std::is_same_v<T, double>, "type must be double");
    constexpr int simd_width = 8; 
                ...
                for (int ii = i; ii < i_end; ++ii)
                {
                    for (int kk = k; kk < k_end; ++kk)
                    {
                        ...
                        for (; jj + 2 * simd_width <= j_end; jj += 2 * simd_width)
                        {
                            ...
                            __m512d res_vec2 = _mm512_loadu_pd(&output[ii * N + jj + simd_width]);
...
template <typename T, int M, int N, int K, int BM, int BN, int BK>
void tsmm_simd_optimized(T *lhs, T *rhs, T *output)
{
    static_assert(std::is_same_v<T, double>, "type must be double");
    constexpr int simd_width = 8; 
...
template <typename T, int M, int N, int K, int BM, int BN, int BK>
void tsmm_simd_optimized2(T *lhs, T *rhs, T *output)
{
    static_assert(std::is_same_v<T, double>, "type must be double");
                ...
                for (int kk = k; kk < k_end; ++kk) {
                    ...
                    for (; j + 4*simd_width <= N; j += 4*simd_width) {
                        ...
                        for (int v = 0; v < 4; ++v) {
                            ...
                            res_vec[v] = _mm512_loadu_pd(&output[ii * N + j + v*simd_width]);
                        }
                        for (int v = 0; v < 4; ++v) {
                            res_vec[v] = _mm512_fmadd_pd(a_vec, b_vec[v], res_vec[v]);
                            _mm512_storeu_pd(&output[ii * N + j + v*simd_width], res_vec[v]);
                        }
                    }
