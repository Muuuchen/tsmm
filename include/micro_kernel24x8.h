...
// 辅助函数：打印 __m512d 向量的8个 double 元素
void print_m512d(__m512d vec, const char *name) {
  double buf[8];
  _mm512_storeu_pd(buf, vec); // 将向量存储到数组
  std::cout << name << ": ";
  for (int i = 0; i < 8; i++) {
    std::cout << buf[i] << " ";
  }
...
inline void MicroKernelImpl<24, 8>(double *A_start, double *B_start,
  ...
  __m512d c = _mm512_loadu_pd(&C_start[0]);
  print_m512d(c, ":sa");
#pragma unroll
  for (int j = 0; j < 8; j++) {
    for (int i = 0; i < 3; i++) {
...
void kernel_tsmm(double *A, double *B, double *C, int M, int N, int K, int lda,
                 int ldb, int ldc) {
  MicroKernelImpl<mr, nr>(address(A, 0, 0, lda), address(B, 0, 0, ldb),
                          address(C, 0, 0, ldc), K, lda, ldb, ldc);
